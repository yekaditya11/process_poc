...
const UnifiedSafetyDashboard = () => {
  ...
  const { setActiveModule, setFilters, setDateRange: setContextDateRange } = useDashboardContext();
  ...
  // Update context when module or filters change
  useEffect(() => {
    console.log('🎯 Setting active module in context:', selectedModule);
    setActiveModule(selectedModule);
  }, [selectedModule]); // Removed setActiveModule from dependencies

  useEffect(() => {
    setContextDateRange({
      isCustom: dateRange.startDate && dateRange.endDate,
  ...
  }, [dateRange]); // Removed setContextDateRange and setFilters from dependencies
