...
import SimpleAIThinkingSteps from './SimpleAIThinkingSteps';
...
const ChatBot = ({ moduleContext = null }) => {
  ...
  const handleSendMessage = async (messageText) => {
    ...
    console.log('🚀 Starting streaming for message:', messageText);
    console.log('🚀 Streaming state:', { isStreaming: true, currentUserMessage: messageText });
    ...
    try {
      ...
      // Note: axios interceptor returns response.data, so response is already the data object
      // Check both the application success flag and HTTP status
      const isHttpSuccess = response._isSuccess || response._httpStatus === 200;
      const isAppSuccess = response && response.success;

      console.log('📊 Response Status Check:', {
        httpStatus: response._httpStatus,
        isHttpSuccess,
        appSuccess: response.success,
        isAppSuccess,
        shouldStopStreaming: isHttpSuccess && isAppSuccess
      });
      ...
      if (isAppSuccess) {
        ...
        // Mark API as complete and let the streaming component finish gracefully
        if (isHttpSuccess && isAppSuccess) {
          console.log('✅ API returned 200 with success=true - marking API as complete');
          setApiStatus('success');
          setIsApiComplete(true);

          // Give the streaming component a moment to show completion, then stop
          setTimeout(() => {
            setIsLoading(false);
          ...
          console.log('⚠️ App success but HTTP status unclear - stopping streaming anyway');
          setApiStatus('success');
          setIsApiComplete(true);
          setTimeout(() => {
            setIsLoading(false);
      ...
      // Stop streaming on error as well
      console.log('❌ API error - stopping streaming');
      setApiStatus('error');
      setIsApiComplete(true);
      setTimeout(() => {
        setIsLoading(false);
            ...
            >
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, zIndex: 1 }}>
                    <motion.div
                      ...
                      transition={{
                        ...
                        scale: { duration: 1, repeat: isLoading ? Infinity : 0, ease: "easeInOut" }
                      }}
                    >
                      <Avatar
                        sx={{
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" sx={{ opacity: 0.8, fontSize: '0.75rem' }}>
                        {isLoading ? 'Processing your request...' : 'Ready to help'}
                      </Typography>
                    </Box>
                  </Box>
              ...
              <Box
                ...
                >
                  ...
                  {/* Messages with Optimized Animations */}
                  <motion.div
                  ...
                  >
                    {messages.map((message, index) => {
                      // Show user messages immediately, only stagger AI responses
                      const isUserMessage = message.role === 'user';
                      const isLatestMessage = index === messages.length - 1;

                      // No delay for user messages or the latest message for immediate feedback
                      const animationDelay = (isUserMessage || isLatestMessage) ? 0 : Math.min(index * 0.05, 0.3);
                  ...
                  {/* Loading States - Simplified Logic */}
                  {isLoading && (
                    <div>
                      {/* Debug info - remove in production */}
                      {process.env.NODE_ENV === 'development' && (
                        <div style={{ fontSize: '10px', color: 'gray', padding: '5px', border: '1px solid red', marginBottom: '8px' }}>
                          Debug: isStreaming={isStreaming.toString()}, currentUserMessage="{currentUserMessage}", isLoading={isLoading.toString()}
                          <br />
                          Streaming condition: {(isStreaming && currentUserMessage).toString()}
                          <br />
                          Should show StreamingThinkingIndicator: {(isStreaming && currentUserMessage && isLoading).toString()}
                        </div>
                      )}

                      {/* Enhanced AI Thinking Process - Always Show When Loading */}
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -20 }}
                      ...
                      >
                        {console.log('🎬 Always rendering SimpleAIThinkingSteps with:', { currentUserMessage, isStreaming, isLoading, isApiComplete, apiStatus })}
                        <SimpleAIThinkingSteps
                          userMessage={currentUserMessage || "Analyzing your safety management query"}
                  ...
                  {/* Test Streaming Button - Development Only */}
                  {process.env.NODE_ENV === 'development' && !isLoading && (
                    <div style={{ padding: '10px', textAlign: 'center' }}>
                      <button
                        onClick={() => {
                          console.log('🧪 Testing enhanced AI thinking steps...');
                          setCurrentUserMessage('Analyze safety performance with comprehensive insights and recommendations');
                          setIsStreaming(true);
                          setIsLoading(true);
                          setTimeout(() => {
                            setIsStreaming(false);
                            setCurrentUserMessage('');
                            setIsLoading(false);
                          }, 10000); // 10 seconds to see full thinking process
                        }}
                        style={{
                          padding: '8px 16px',
                          fontSize: '12px',
