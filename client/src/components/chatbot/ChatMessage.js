...
const ChatMessage = ({
  ...
  if (!isUser && message.chart_data) {
    console.log('📊 Chart data received in ChatMessage:', message.chart_data);
    console.log('📊 Has chart:', hasChart);
    console.log('📊 Chart type:', message.chart_data.type);
    console.log('📊 Chart data array:', message.chart_data.data);
    console.log('📊 Chart data length:', message.chart_data.data?.length);
    
    // Check if data is properly formatted for line charts
    if (message.chart_data.type === 'line' && message.chart_data.data) {
      console.log('📊 Line chart data validation:');
      message.chart_data.data.forEach((item, index) => {
        console.log(`  Point ${index}:`, {
          name: item.name || item.label || item.x,
          value: item.value || item.y,
          hasName: !!(item.name || item.label || item.x),
          hasValue: item.value !== undefined || item.y !== undefined
        });
      });
    }
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Chart Section (moved before suggested actions) */}
          {hasChart && (
          ...
          {/* Suggested Actions Section - Follow-up questions (moved after chart) */}
