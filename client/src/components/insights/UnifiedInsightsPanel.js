...
import SimpleAIThinkingSteps from '../chatbot/SimpleAIThinkingSteps';
import ApiService from '../../services/api';
import chartManager from '../../services/chartManager';

// Import AI Insights Streaming component
import AIInsightsStreaming from './AIInsightsStreaming';
...
const UnifiedInsightsPanel = ({
  ...
  const [isStreaming, setIsStreaming] = useState(false); // AI Insights streaming state
  const [streamingModule, setStreamingModule] = useState(null); // Current module being analyzed
  const [isChatLoading, setIsChatLoading] = useState(false); // Separate chat loading state
  ...
  // Handle AI Insights streaming completion
  const handleStreamingComplete = (result) => {
    console.log('🎬 AI Insights streaming completed:', result);
    setIsStreaming(false);
    setStreamingModule(null);
  };

  // Start streaming when AI loading begins - completely separate from chat
  useEffect(() => {
    if (aiLoading && selectedModule && activeTab === 0 && !isStreaming) {
      console.log('🎬 Starting AI Insights streaming for module:', selectedModule);
      setIsStreaming(true);
      setStreamingModule(selectedModule);
    }
  }, [aiLoading, selectedModule, activeTab, isStreaming]);

  // Stop AI insights streaming when AI loading stops
  useEffect(() => {
    if (!aiLoading && isStreaming) {
      console.log('🎬 Stopping AI Insights streaming - AI loading stopped');
      setIsStreaming(false);
      setStreamingModule(null);
    }
  }, [aiLoading, isStreaming]);

  // Stop AI insights streaming when switching to chat tab
  useEffect(() => {
    if (activeTab === 1 && isStreaming) {
      console.log('🎬 Stopping AI Insights streaming - switching to chat tab');
      setIsStreaming(false);
      setStreamingModule(null);
    }
        ...
        <CardContent sx={{
          ...
          {activeTab === 0 && (
            <Box sx={{
              ...
              {aiLoading ? (
                ...
                >
                  {/* AI Insights Streaming Component - only show on AI insights tab */}
                  {isStreaming && streamingModule && activeTab === 0 ? (
                    <AIInsightsStreaming
                      module={streamingModule}
                      isVisible={isStreaming && activeTab === 0}
          ...
          {activeTab === 1 && (
            <Box sx={{
              ...
              >
                {/* Welcome Message - shown as regular chat message */}
                ...
                {/* Enhanced AI Thinking Steps - only show in chat tab */}
                {isChatLoading && activeTab === 1 && (
                  <motion.div
                  ...
                  >
                    <SimpleAIThinkingSteps
                      userMessage="Generating comprehensive safety insights and analysis"
                    />
                  </motion.div>
                )}
