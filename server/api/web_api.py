...
@app.get("/debug/line-chart-test")
async def debug_line_chart_test():
    """Test endpoint to generate a simple line chart for debugging"""
    try:
        # Create a simple line chart test data
        test_data = {
            "type": "line",
        ...
        chart_config = echarts_gen.generate_chart(test_data)
        
        return {
            "success": True,
            "message": "Test line chart generated",
            "chart_data": chart_config,
            "test_data": test_data
        }
        
    except Exception as e:
        logger.error(f"Error in line chart test: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def generate_similar_insights(module: str, positive_insight: str):
    ...
    try:
        ...
        # Call Azure OpenAI to generate similar insights
        deployment_name = summarizer_app.ai_engine._get_azure_deployment_name(optimal_model)
...
async def get_module_data_for_insights(module: str):
    ...
    try:
        ...
        if module == 'global-dashboard':
            # Aggregate data from all modules for comprehensive dashboard insights
            end_date = datetime.now()
            start_date = end_date - timedelta(days=30)
            ...
            try:
                all_module_data['incident_investigation'] = summarizer_app.incident_extractor.get_all_incident_kpis(
                    customer_id=None, start_date=start_date, end_date=end_date
                )
            except Exception as e:
                logger.warning(f"Error getting incident investigation data: {str(e)}")
                all_module_data['incident_investigation'] = {}
            
            # Action tracking data
            try:
                all_module_data['action_tracking'] = summarizer_app.action_extractor.get_all_action_tracking_kpis(
                    customer_id=None, start_date=start_date, end_date=end_date
                )
            except Exception as e:
                logger.warning(f"Error getting action tracking data: {str(e)}")
                all_module_data['action_tracking'] = {}
            
            # Driver safety data
            try:
                all_module_data['driver_safety'] = summarizer_app.driver_safety_extractor.get_driver_safety_checklist_kpis(
                    customer_id=None, days_back=30
                )
            except Exception as e:
                logger.warning(f"Error getting driver safety data: {str(e)}")
                all_module_data['driver_safety'] = {}
            
            # Observation tracker data
            try:
                all_module_data['observation_tracker'] = summarizer_app.observation_tracker_extractor.get_observation_tracker_kpis(
                    customer_id=None, days_back=30
                )
            except Exception as e:
                logger.warning(f"Error getting observation tracker data: {str(e)}")
                all_module_data['observation_tracker'] = {}
            
            # Equipment asset data
            try:
                all_module_data['equipment_asset'] = summarizer_app.equipment_asset_extractor.get_equipment_asset_kpis(
                    customer_id=None, days_back=30
                )
            except Exception as e:
                logger.warning(f"Error getting equipment asset data: {str(e)}")
                all_module_data['equipment_asset'] = {}
            
            # Employee training data
            try:
                all_module_data['employee_training'] = summarizer_app.employee_training_extractor.get_employee_training_kpis(
                    customer_id=None, days_back=365
                )
            except Exception as e:
                logger.warning(f"Error getting employee training data: {str(e)}")
                all_module_data['employee_training'] = {}
            
            # Risk assessment data
            try:
                all_module_data['risk_assessment'] = summarizer_app.risk_assessment_extractor.get_risk_assessment_kpis(
                    customer_id=None, days_back=30
                )
            except Exception as e:
                logger.warning(f"Error getting risk assessment data: {str(e)}")
                all_module_data['risk_assessment'] = {}
            
            return all_module_data

        elif module == 'incident-investigation':
...
def extract_data_points_for_analysis(module: str, module_data: dict):
    ...
    try:
        ...
        if module == 'global-dashboard':
            # Extract data from all modules for comprehensive dashboard analysis
                    ...
                    if isinstance(module_info, dict):
                        ...
                        for key, value in module_info.items():
                            if isinstance(value, (int, float)):
                                data_points.append(f"{module_name} - {key.replace('_', ' ').title()}: {value}")
                            elif isinstance(value, dict):
                                for sub_key, sub_value in value.items():
                                    if isinstance(sub_value, (int, float, str)):
                                        data_points.append(f"{module_name} - {sub_key.replace('_', ' ').title()}: {sub_value}")
        ...
        result = "\n".join(data_points[:50])  # Increased limit for global dashboard
...
def generate_data_driven_fallback_insights(module: str, module_data: dict, count: int = 5):
    ...
    try:
        ...
        if module == 'global-dashboard' and module_data:
            # Generate comprehensive insights from all modules
            if 'incident_investigation' in module_data:
                incident_data = module_data['incident_investigation']
                if 'incidents_reported' in incident_data:
                    insights.append({"text": f"Global incident count: {incident_data['incidents_reported']} incidents reported", "sentiment": "neutral"})

            if 'action_tracking' in module_data:
                action_data = module_data['action_tracking']
                if 'on_time_completion' in action_data and isinstance(action_data['on_time_completion'], dict):
                    completion_rate = action_data['on_time_completion'].get('on_time_percentage', 0)
                    insights.append({"text": f"Global action completion rate: {completion_rate}% on-time completion", "sentiment": "positive" if completion_rate > 80 else "negative"})

            if 'driver_safety' in module_data:
                driver_data = module_data['driver_safety']
                if 'daily_completions' in driver_data and isinstance(driver_data['daily_completions'], dict):
                    completion_rate = driver_data['daily_completions'].get('completion_percentage', 0)
                    insights.append({"text": f"Driver safety checklist completion: {completion_rate}% daily completion rate", "sentiment": "positive" if completion_rate > 90 else "neutral"})

            if 'employee_training' in module_data:
                training_data = module_data['employee_training']
                if 'expired_trainings' in training_data and isinstance(training_data['expired_trainings'], dict):
                    expired_count = training_data['expired_trainings'].get('expired_count', 0)
                    insights.append({"text": f"Employee training compliance: {expired_count} expired trainings requiring attention", "sentiment": "negative" if expired_count > 0 else "positive"})

        elif module == 'incident-investigation' and module_data:
...
async def generate_additional_insights(module: str, existing_insights: list, positive_examples: list, count: int = 5):
    ...
    try:
        ...
        deployment_name = summarizer_app.ai_engine._get_azure_deployment_name("gpt-3.5-turbo")
        response = summarizer_app.ai_engine.openai_client.chat.completions.create(
            model=deployment_name,
            messages=[
...
def generate_fallback_additional_insights(module: str, count: int = 5):
    ...
    fallback_insights = {
        'global-dashboard': [
            {"text": "Cross-module integration reveals comprehensive safety performance patterns", "sentiment": "positive"},
            {"text": "Unified dashboard provides holistic view of organizational safety health", "sentiment": "positive"},
            {"text": "Inter-module correlations identify systemic safety improvement opportunities", "sentiment": "positive"},
            {"text": "Comprehensive data aggregation enables strategic safety decision-making", "sentiment": "positive"},
            {"text": "Multi-module analytics support evidence-based safety program optimization", "sentiment": "positive"}
        ],
