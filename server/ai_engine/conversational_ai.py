...
class ConversationalAI:
    ...
    def __init__(self, summarizer_app):
        ...
        # Initialize Azure OpenAI client with high-performance settings
        ...
        if openai_available:
            ...
            if api_key and endpoint:
                ...
                logger.info("Azure OpenAI client initialized with high-performance settings")
            else:
                logger.warning("Azure OpenAI API key or endpoint not found")
        else:
        ...
        try:
            ...
            if self.openai_client and has_meaningful_data:
                ...
                try:
                    ...
                    deployment_name = self._get_azure_deployment_name(optimal_model)
...
Direct Answer to "{clean_user_message}":"""
    ...
    def _get_azure_deployment_name(self, model_name: str) -> str:
        """Map OpenAI model names to Azure deployment names - using single model"""
        # Use the single deployment for all model requests
    ...
    def _summarize_data_context(self, data_context: Dict[str, Any]) -> str:
        ...
        if 'observations' in data_context:
            ...
            total_observations = observations.get('total_observations', 0)
    ...
    def _has_meaningful_data(self, data_context: Dict[str, Any]) -> bool:
        ...
        for module_name, module_data in data_context.items():
            ...
            if module_name == 'incidents':
                ...
                # Check for total_observations at the top level first, then fallback to nested structure
                total_observations = module_data.get('total_observations', 0)
                if total_observations == 0:
    ...
    def _generate_ai_chart_spec(self, user_message: str, data_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            Create a chart with proper labels and values. Respond with ONLY valid JSON in this exact format:
            {{
                "create_chart": true,
                "type": "bar",
                "title": "Descriptive Chart Title",
                "data": [
                    {{"name": "Category Name", "value": 123}},
                    {{"name": "Another Category", "value": 456}}
                ]
            }}
            ...
            - Extract real data from the provided data context
            - If no chart needed, respond with: {{"create_chart": false}}
            - Chart types: bar, donut, line, pie, scatter
            """
            ...
            # Map OpenAI model names to Azure deployment names
            deployment_name = self._get_azure_deployment_name(chart_model)
            ...
            logger.info(f"AI chart response: {ai_response[:200]}...")
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                try:
                    ...
                    logger.info(f"Successfully parsed AI chart spec: {chart_spec.get('type', 'unknown')} chart")
                    ...
                    if chart_spec.get('create_chart', False):
                        ...
                        logger.info("AI determined no chart should be created")
                        return None
                except json.JSONDecodeError as json_error:
                    logger.error(f"JSON parsing error in AI chart response: {str(json_error)}")
                    logger.error(f"Problematic JSON: {json_str[:500]}...")
                    # Try to fix common JSON issues
                    try:
                        # Remove trailing commas and fix common issues
                        fixed_json = json_str.replace(',}', '}').replace(',]', ']')
                        chart_spec = json.loads(fixed_json)
                        logger.info("Successfully parsed AI chart spec after JSON fix")
                        ...
                        logger.error("Failed to fix JSON, falling back to manual chart generation")
            else:
                logger.error("No valid JSON found in AI response")

        except Exception as e:
            logger.error(f"Error in AI chart generation: {str(e)}")
            logger.info("AI chart generation failed, will use fallback chart generation")
        ...
        if has_data:
            ...
            if any(keyword in user_message_lower for keyword in ['observation', 'area', 'behavior', 'unsafe', 'safe']):
                if 'observations' in data_context:
                    ...
                    # Check for total_observations in multiple places
                    total_observations = obs_data.get('total_observations', 0)
                    if total_observations == 0:
                        total_observations = obs_data.get('summary', {}).get('total_observations', 0)
                    if total_observations == 0:
                        total_observations = obs_data.get('observation_status', {}).get('total_observations', 0)
                    if total_observations == 0:
            ...
            if 'observations' in data_context:
                ...
                if total_observations == 0:
                    total_observations = obs_data.get('observations_by_area', {}).get('total_observations', 0)
                if total_observations > 0:
                    summary_parts.append(f"Observations: {total_observations} total")
