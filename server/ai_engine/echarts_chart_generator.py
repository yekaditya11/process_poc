...
class EChartsGenerator:
    ...
    def _create_line_chart(self, title: str, data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, item in enumerate(data):
                name = item.get('name', item.get('label', f'Point {i}'))
                value = item.get('value', item.get('y', 0))
            ...
            logger.info(f"Line chart data processed: {len(categories)} categories, {len(values)} values")
            logger.info(f"Sample data: categories={categories[:3]}, values={values[:3]}")
            
            if not categories or not values:
                logger.error("No valid data for line chart")
                return None
            ...
            logger.info(f"Line chart config generated successfully for title: {title}")
